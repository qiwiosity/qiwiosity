...
const STORAGE_KEY = '@qiwiosity/itinerary';
...
export function ItineraryProvider({ children }) {
  ...
  const api = {
    items: state.items,
    /** Add a POI to the itinerary. Automatically tagged itemType:'poi'. */
    add: (item) => dispatch({ type: 'add', item: { ...item, itemType: item.itemType || 'poi' } }),
    /** Add an accommodation to the itinerary. Automatically tagged itemType:'accommodation'. */
    addAccommodation: (item) => dispatch({ type: 'add', item: { ...item, itemType: 'accommodation' } }),
    remove: (id) => dispatch({ type: 'remove', id }),
    reorder: (items) => dispatch({ type: 'reorder', items }),
    clear: () => dispatch({ type: 'clear' }),
    has: (id) => state.items.some((x) => x.id === id),
    /** Get only POI items */
    pois: state.items.filter((x) => (x.itemType || 'poi') === 'poi'),
    /** Get only accommodation items */
    stays: state.items.filter((x) => x.itemType === 'accommodation'),
  };
